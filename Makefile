...
build-noble-ansible-qemu-amd: validate  validate-cloudinit
	PACKER_LOG=1 packer build -var arch="amd" -var-file=${NOBLE_ANSIBLE_VARS_FILE} -only=base.qemu.base ${QEMU_FOLDER}

build-noble-ansible-qemu-arm: validate
	PACKER_LOG=1 packer build -var arch="arm" -var-file=${NOBLE_ANSIBLE_VARS_FILE} -only=base.qemu.base ${QEMU_FOLDER}

build-noble-k3s-server-qemu-amd: validate  validate-cloudinit
	PACKER_LOG=1 packer build -var arch="amd" -var-file=${NOBLE_K3S_SERVER_VARS_FILE} -only=ansible.qemu.base ${QEMU_FOLDER}

build-noble-k3s-server-qemu-arm: validate
	PACKER_LOG=1 packer build -var arch="arm" -var-file=${NOBLE_K3S_SERVER_VARS_FILE} -only=ansible.qemu.base ${QEMU_FOLDER}

build-noble-k3s-agent-qemu-amd: validate  validate-cloudinit
	PACKER_LOG=1 packer build -var arch="amd" -var-file=${NOBLE_K3S_AGENT_VARS_FILE} -only=ansible.qemu.base ${QEMU_FOLDER}

build-noble-k3s-agent-qemu-arm: validate
	PACKER_LOG=1 packer build -var arch="arm" -var-file=${NOBLE_K3S_AGENT_VARS_FILE} -only=ansible.qemu.base ${QEMU_FOLDER}

build-noble-k3s-istio-qemu-arm: validate
	PACKER_LOG=1 packer build -var arch="arm" -var-file=${NOBLE_K3S_ISTIO_VARS_FILE} -only=ansible-remote.null.ansible ${QEMU_FOLDER}
